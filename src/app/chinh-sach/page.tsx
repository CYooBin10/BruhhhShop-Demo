...
export default function PoliciesPage() {
  return <><main className="policy-page policy-index"><div className="container policy-shell"><Link className="policy-back" href="/#bang-gia">← Quay lại bảng giá</Link><div className="policy-heading"><p className="section-label">Minh bạch trước khi mua</p><h1>Chính sách &<br /><span>Điều khoản</span></h1><p>Đọc thông tin về sử dụng VPS, bảo hành, hoàn tiền, thanh toán và quyền riêng tư trước khi đặt hàng.</p><span className="policy-updated">Cập nhật lần cuối: {businessConfig.lastPolicyUpdate}</span></div><div className="policy-index-list">{policyLinks.map((policy) => <Link className="policy-index-item" href={`/chinh-sach/${policy.slug}`} key={policy.slug}><span><strong>{policy.label}</strong><small>Xem nội dung chi tiết</small></span><span>→</span></Link>)}</div></div></main><Footer /></>;
}

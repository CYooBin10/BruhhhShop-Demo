...
function PricingCard({ plan, promotion, onOrder }: { plan: Plan; promotion?: PlanPromotionConfig; onOrder: (plan: Plan) => void }) {
  ...
  return <article className={`pricing-card ${plan.popular ? "is-popular" : ""}`}>{plan.popular ? <div className="popular-ribbon">Bán chạy <Icon name="spark" /></div> : null}<div className="plan-top">{discountPercent ? <span className="discount-badge">-{discountPercent}%</span> : null}<span className={`availability ${outOfStock ? "is-out" : ""}`}><i /> {plan.status}</span></div><h3>{plan.name}</h3><div className={`plan-price ${hasPromotion ? "has-promotion" : ""}`}>{hasPromotion ? <s>{formatVnd(originalPrice)}</s> : null}<div className="plan-sale"><strong>{displayedPrice}</strong><span>/{plan.period}</span></div></div><div className="plan-rule" /><ul className="plan-features"><li><span><Icon name="cpu" /> vCPU</span><strong>{plan.cpu}</strong></li><li><span><Icon name="server" /> RAM</span><strong>{plan.ram}</strong></li><li><span><Icon name="cpu" /> CPU</span><strong>{business.infrastructure.cpuModel}</strong></li><li><span><Icon name="server" /> Ổ đĩa</span><strong>{plan.storage}</strong></li><li><span><Icon name="globe" /> Băng thông</span><strong>{plan.portSpeed}</strong></li><li><span><Icon name="shield" /> Anti-DDoS</span><strong>{business.infrastructure.ddosProtection}</strong></li></ul><button className={`button plan-button ${plan.popular ? "button-primary" : "button-ghost"}`} disabled={outOfStock} type="button" onClick={() => onOrder(plan)}>{outOfStock ? "Tạm hết hàng" : "Đăng ký gói này"} <Icon name="arrow-up-right" /></button></article>;
}

...
function PricingCard({ plan, promotion, onOrder }: { plan: Plan; promotion?: PlanPromotionConfig; onOrder: (plan: Plan) => void }) {
  ...
  return <article className={`pricing-card ${plan.popular ? "is-popular" : ""}`}><div className="plan-top"><span className="plan-type">{plan.popular ? <><Icon name="spark" /> Gói nổi bật</> : "Cloud VPS"}</span><span className={`availability ${outOfStock ? "is-out" : ""}`}><i /> {plan.status}</span></div><h3>{plan.name}</h3><div className="plan-resources"><div><strong>{plan.cpu.replace(" vCPU", "")}</strong><span>vCPU</span></div><div><strong>{plan.ram.replace(" GB", "")}</strong><span>GB RAM</span></div><div><strong>{plan.storage.replace(" GB NVMe", "")}</strong><span>GB NVMe</span></div></div><div className="plan-price"><div className="plan-price-meta">{hasPromotion ? <><s>{formatVnd(originalPrice)}</s><span className="discount-badge">Giảm {discountPercent}%</span></> : <span className="standard-price">Thanh toán theo tháng</span>}</div><div className="plan-sale"><strong>{displayedPrice}</strong><span>/{plan.period}</span></div></div><ul className="plan-features"><li><span>CPU</span><strong>{business.infrastructure.cpuModel}</strong></li><li><span>Cổng mạng</span><strong>{plan.portSpeed}</strong></li><li><span>Anti-DDoS</span><strong>{business.infrastructure.ddosProtection}</strong></li></ul><button className={`button plan-button ${plan.popular ? "button-primary" : "button-ghost"}`} disabled={outOfStock} type="button" onClick={() => onOrder(plan)}>{outOfStock ? "Tạm hết hàng" : "Chọn gói này"} <Icon name="arrow-up-right" /></button></article>;
}

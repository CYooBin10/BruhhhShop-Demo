...
function PricingCard({ plan, promotion, onOrder }: { plan: Plan; promotion?: PlanPromotionConfig; onOrder: (plan: Plan) => void }) {
  ...
  return <article className={`pricing-card ${plan.popular ? "is-popular" : ""}`}><div className="plan-top"><span className="plan-type">{plan.popular ? <><Icon name="spark" /> Phổ biến</> : "VPS NVMe"}</span><span className={`availability ${outOfStock ? "is-out" : ""}`}><i /> {plan.status}</span></div><h3>{plan.name}</h3><div className="plan-price"><div className="plan-price-meta">{hasPromotion ? <><s>{formatVnd(originalPrice)}</s><span className="discount-badge">-{discountPercent}%</span></> : <span className="standard-price">Giá tiêu chuẩn</span>}</div><div className="plan-sale"><strong>{displayedPrice}</strong><span>/{plan.period}</span></div></div><ul className="plan-features"><li><span>Vi xử lý</span><strong>{plan.cpu}</strong></li><li><span>Bộ nhớ</span><strong>{plan.ram}</strong></li><li><span>Dòng CPU</span><strong>{business.infrastructure.cpuModel}</strong></li><li><span>Lưu trữ</span><strong>{plan.storage}</strong></li><li><span>Cổng mạng</span><strong>{plan.portSpeed}</strong></li><li><span>Bảo vệ</span><strong>{business.infrastructure.ddosProtection}</strong></li></ul><button className={`button plan-button ${plan.popular ? "button-primary" : "button-ghost"}`} disabled={outOfStock} type="button" onClick={() => onOrder(plan)}>{outOfStock ? "Tạm hết hàng" : "Đăng ký gói này"} <Icon name="arrow-up-right" /></button></article>;
}
